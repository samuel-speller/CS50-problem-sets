...
bool check(char *word)
{
    //set cursor as a pointer to the root node
    node *cursor = root;

    //iterate over the word and check against the trie
    for (int i = 0; i < strlen(word); i++)
    {
        //make sure the input word is all in lowercase
        word[i] = tolower(word[i]);

        //check for node pointer
        int index = word[i] - 'a';
        if (cursor->children[index] == NULL)
        {
            return false;
    ...
    //check if the node cursor is pointing to has got a true is_word
